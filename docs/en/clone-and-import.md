...
`git clone https://github.com/ByteLegendQuest/java-fix-counting-hash-set`

A project directory named `java-fix-counting-hash-set` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-counting-hash-set` directory which is created just now.
